...
const WorkspaceSidebar: React.FC<UniversalSidebarProps> = ({
  locationPath = '',
  ...
  const section = 'workspace' as const;
  ...
  const { title, items, backLink } = useMemo(() => {
    return {
      ...
      items: [
        ...
        { icon: Grid, label: 'Spaces', href: '/workspace/spaces' },
  ...
  // Suppress active highlighting for Home
  const suppressedActiveHrefs = useMemo(() => new Set(['/workspace']), []);
