...
exports.deleteWorkspace = async (req, res) => {
    try {
        ...
        // Get workspace with members populated for cleanup
        const workspace = await Workspace.findById(workspaceId)
        ...
        if (workspace.owner.toString() !== userId) {
            return sendResponse(res, 403, false, 'Only the workspace owner can delete this workspace');
        }
        ...
        // Remove workspace roles from all members
        const memberIds = workspace.members.map(m => m.user._id);
        await User.updateMany(
            { _id: { $in: memberIds } },
            { $pull: { 'roles.workspaces': { workspace: workspaceId } } }
        );

        // Delete the workspace
        await Workspace.findByIdAndDelete(workspaceId);

        // Clean up related data (invitations, activity logs, etc.)
        await Promise.all([
            Invitation.deleteMany({ 'targetEntity.id': workspaceId }),
            // Add other related collections that need cleanup
        ]);

        logger.info(`Workspace ${workspaceId} deleted by user ${userId}`);
        
        sendResponse(res, 200, true, 'Workspace deleted successfully');
    } catch (error) {
        logger.error('Delete workspace error:', error);
        sendResponse(res, 500, false, 'Server error deleting workspace');
    }
};
