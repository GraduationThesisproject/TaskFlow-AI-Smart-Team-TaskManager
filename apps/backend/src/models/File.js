  ...
  fileSchema.statics.createFromUpload = function(multerFile, uploadedBy, category = 'general', additionalData = {}) {
    ...
    const fileData = {
      ...
      category: category,
      ...additionalData // Spread additional data (including attachedTo if provided)
    };
    
    // If attachedTo is not provided in additionalData, set default for User avatars
    if (!fileData.attachedTo && category === 'avatar' && uploadedBy) {
      fileData.attachedTo = {
        model: 'User',
        objectId: uploadedBy,
        attachedAt: new Date()
