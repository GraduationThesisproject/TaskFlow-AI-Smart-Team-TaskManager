...
function SpaceMainScreenContent() {
  ...
  const filteredBoards = useMemo(() => {
    const list = (Array.isArray(boardsFromHook) && boardsFromHook.length > 0)
      ? boardsFromHook
  ...
  const totalBoardsCount = useMemo(() => {
    return (Array.isArray(boardsFromHook) && boardsFromHook.length > 0)
      ? boardsFromHook.length
      : (Array.isArray(boards) ? boards.length : 0);
  }, [boardsFromHook, boards]);

  const totalTasksCount = useMemo(() => {
    const source = (Array.isArray(boardsFromHook) && boardsFromHook.length > 0)
      ? boardsFromHook
      : (Array.isArray(boards) ? boards : []);
    return source.reduce((sum: number, b: any) => {
      const t = Number(b?.tasksCount || b?.totalTasks || 0);
      ...
      <View style={{ flex: 1 }}>
        ...
        <SpaceHeader
          space={{
            ...space,
            members: spaceMembers,
            totalBoards: Array.isArray(boardsFromHook) && boardsFromHook.length > 0 ? boardsFromHook.length : (Array.isArray(boards) ? boards.length : (space?.stats?.totalBoards || 0)),
            stats: { ...(space?.stats || {}), totalBoards: Array.isArray(boardsFromHook) && boardsFromHook.length > 0 ? boardsFromHook.length : (Array.isArray(boards) ? boards.length : (space?.stats?.totalBoards || 0)) },
          }}
          boardCount={(Array.isArray(boardsFromHook) && boardsFromHook.length > 0) ? boardsFromHook.length : (Array.isArray(boards) ? boards.length : 0)}
          membersCount={Array.isArray(spaceMembers) ? spaceMembers.length : 0}
        ...
        {loading && !refreshing ? (
          ...
          <>
            {Array.isArray(filteredBoards) && filteredBoards.length > 0 ? (
              ...
              <Text style={[TextStyles.body.medium, { color: colors['muted-foreground'], textAlign: 'center', paddingVertical: 12 }]}>No results.</Text>
            ) : (
