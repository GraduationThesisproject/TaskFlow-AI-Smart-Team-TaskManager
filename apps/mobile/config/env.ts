...
// Simple network configuration with IP management
const getNetworkConfig = () => {
  ...
  const IPS = ['10.76.16.1',"10.72.191.13",'192.168.1.115','192.168.1.14','192.168.1.64'];
  // Force use the correct IP for now
  const CURRENT_IP = '10.72.191.13';
  
  // Debug logging
  if (__DEV__) {
    console.log('🔧 IP Configuration Debug:', {
      selectedIP: CURRENT_IP,
      fallbackIPs: IPS
    });
  }
  ...
  if (isDev) {
    if (isAndroid) {
      ...
      return {
        apiBaseUrl: `http://${CURRENT_IP}:3001/api`,
        baseUrl: `http://${CURRENT_IP}:3001`,
        socketUrl: `http://${CURRENT_IP}:3001`,
        frontendUrl: `http://${CURRENT_IP}:5173`,
      };
    }
...
export const env = {
  ...
  // Dev auth helper (for local testing of protected endpoints)
  // Set via app.json -> expo.extra.devToken or EXPO_PUBLIC_DEV_TOKEN
  DEV_TOKEN: process.env.EXPO_PUBLIC_DEV_TOKEN || (Constants.expoConfig as any)?.extra?.devToken || '',
  DEV_AUTH_TOKEN: process.env.EXPO_PUBLIC_DEV_AUTH_TOKEN || (Constants.expoConfig as any)?.extra?.devAuthToken || '',
  API_DEV_TOKEN: process.env.EXPO_PUBLIC_API_DEV_TOKEN || (Constants.expoConfig as any)?.extra?.apiDevToken || '',
  
  // OAuth Configuration
  GOOGLE_CLIENT_ID: process.env.EXPO_PUBLIC_GOOGLE_CLIENT_ID || Constants.expoConfig?.extra?.googleClientId || '',
  GOOGLE_CLIENT_SECRET: process.env.EXPO_PUBLIC_GOOGLE_CLIENT_SECRET || Constants.expoConfig?.extra?.googleClientSecret || '',
  ...
  GITHUB_CLIENT_ID: process.env.EXPO_PUBLIC_GITHUB_CLIENT_ID || Constants.expoConfig?.extra?.githubClientId || '',
  GITHUB_CLIENT_SECRET: process.env.EXPO_PUBLIC_GITHUB_CLIENT_SECRET || Constants.expoConfig?.extra?.githubClientSecret || '',
